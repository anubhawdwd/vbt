...
search.addEventListener("input", function () {
  ...
  const filteredData = tableDataAPI.filter((row) => {
    return (
...
// Track current sorting order for attributes
const sortOrder = {
  method: "asc",
  status: "asc",
};
...
function sortBy(attribute) {
  ...
  const order = sortOrder[attribute];

  // Sort table data based on the selected attribute and order
  tableData.sort((a, b) => {
  ...
  sortOrder[attribute] = order === "asc" ? "desc" : "asc";
