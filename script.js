...
//1. Serarch Functionality Implementation 
// Event listener for the search input
search.addEventListener('input', function () {
  const searchValue = search.value.toLowerCase();
// console.log(searchValue);

  // Filter table data based on the search value
  const filteredData = tableData.filter(row => {
    return (
      row.method.toLowerCase().includes(searchValue) ||
      row.raga.toLowerCase().includes(searchValue) ||
      (row.sno && row.sno.toString().includes(searchValue)) ||
      (row.vbtSno && row.vbtSno.toString().includes(searchValue)) ||
      row.status.toLowerCase().includes(searchValue)
    );
  });
  // Add animation to rows that will be removed
  const rows = document.querySelectorAll('tbody tr');
  rows.forEach(row => {
    row.classList.add('fade-out'); // Add fade-out class for animation
    setTimeout(() => {
      row.remove(); // Remove rows after animation
    }, 500); // Match duration with CSS animation
  });
 // Render the filtered data after a small delay to allow removal animation
 setTimeout(() => {
...
// 2. Sorting | Ordering data of HTML table
  function sortTableData (columnIndex, sortAscending){
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const sortOptions = document.querySelectorAll(".sort__menu-options li");
    let sortAscending = true; // Toggle to switch between ascending and descending order
  
    // Add event listener to each sort option
    sortOptions.forEach((option) => {
      option.addEventListener("click", () => {
        const columnIndex = parseInt(option.dataset.sort, 10); // Get the column index from data-sort
        sortTableData(columnIndex, sortAscending); // Call the sort function with current direction
...
tableHeadings.forEach((heading, columnIndex) => {
  ...
  // console.log(columnIndex, heading);
  
    let sortAscending = true; // Default sorting order is ascending

    heading.addEventListener('click', () => {
        heading.classList.toggle('asc');
        // Toggle sorting order
        sortAscending = !sortAscending;

        // Sort tableData array directly
...
const methods_table = document.querySelector('#methods_table');

const toPDF = function (methods_table) {
    const html_code = `
    <!DOCTYPE html>
    <link rel="stylesheet" type="text/css" href="style.css">
    <main class="table" id="methods_table">${methods_table.innerHTML}</main>`;
...
function renderTable(data, searchValue) {
  ...
  data.forEach((row, index) => {
    ...
    tr.className = `status-${row.status.toLowerCase().replace(/ /g, "-")} fade-in`;
 
    tr.innerHTML = `
        <td data-label="S.No">${highlightMatch(row.sno, searchValue)}</td>
        <td data-label="Methods">${highlightMatch(row.method, searchValue)}</td>
        <td data-label="VBT S.No">${highlightMatch(row.vbtSno, searchValue)}</td>
        <td class="raganame" data-label="Raga Name">${highlightMatch(row.raga, searchValue)}</td>
        <td data-label="Song Link"><a href="${row.song}" target="_blank">${row.song ? "Listen" : "N/A"}</a></td>
        <td data-label="status">
          <select class="select" onchange="updateStatus(this)" data-row="${index}">
    ...
    // Remove fade-in class after the animation duration
    setTimeout(() => tr.classList.remove('fade-in'), 500);
  });
}

//for highlishting serched term in output Table
